...
export default function CartPage() {
    ...
    return (
        <div className="cart-page container mx-auto p-6 min-h-screen bg-gray-100">
            ...
            <button
                className="bg-yellow-500 text-black font-semibold py-2 px-4 rounded-full hover:bg-yellow-600 transition duration-300 mb-4">Pagar
                (Próximamente)
            </button>
            <button onClick={() => window.location.href = "/productos"}
                    className="bg-gray-700 text-white font-semibold py-2 px-4 rounded-full hover:bg-gray-800 transition duration-300 mb-6">Continuar
                comprando
            </button>
                    ...
                    <div key={item.id} className="cart-item flex items-center bg-white p-4 rounded-lg shadow-lg">
                        ...
                        <div className="flex items-center space-x-3">
                            <button onClick={() => dispatch(incrementItem(item.id))}
                                    className="bg-yellow-500 text-black font-semibold py-1 px-2 rounded-full">+
                            </button>
                            <button onClick={() => dispatch(decrementItem(item.id))}
                                    className="bg-yellow-500 text-black font-semibold py-1 px-2 rounded-full">-
                            </button>
                            <button onClick={() => dispatch(removeItem(item.id))}
                                    className="bg-red-500 text-white font-semibold py-1 px-4 rounded-full">Eliminar
                            </button>
                        </div>
                    </div>
                ))}
